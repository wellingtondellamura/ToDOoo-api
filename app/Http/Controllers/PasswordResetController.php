...
class PasswordResetController extends Controller
{
    ...
    public function create(PasswordResetCreateRequest $request)
    {
        ...
        if (!$user){
            return $this->sendError("Não foi encontrado um usuário com este endereço de e-mail", Response::HTTP_NOT_FOUND);
        }
        $passwordReset = PasswordReset::updateOrCreate(
            ['email' => $user->email],
            [
                'email' => $user->email,
                'token' => Str::random(30).$user->id.Str::random(30)
             ]
        );
        if ($user && $passwordReset){
            $user->notify( new PasswordResetRequest($passwordReset->token));
            return $this->sendMessage("Um e-mail para a redefinição de senha foi enviado.");
        }
    }
